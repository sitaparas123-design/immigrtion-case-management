...
const getRequiredPermission = (method: string, path: string, baseUrl: string): string | null => {
  const fullPath = (baseUrl + path).toLowerCase();
  
  if (fullPath.includes('/clients')) {
    if (method === 'POST') return 'create_client';
    if (method === 'GET') return 'view_clients';
    if (method === 'PUT') return 'edit_client';
    if (method === 'DELETE') return 'delete_client';
  }
  
  if (fullPath.includes('/cases')) {
    if (method === 'POST') return 'case.create';
    if (method === 'GET') return 'case.view';
    if (method === 'PATCH') return 'case.view';
  }
